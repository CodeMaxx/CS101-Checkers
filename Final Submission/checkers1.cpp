...
Board::Board(int a,int b,bool isoc,occupied_by oc)  //'bool isoc'stores if a Box is occupied or not
: cen(a,b)
{
	is_occupied=isoc;
	ocby=oc;
...
Pieces::Pieces(int a,int b,int r)
: cen(a,b)
{
	aux=cen.convert();
...
void Pieces::move(Centre end)			//State of the Box is modified according to the moving Pieces
{	
...
void Pieces::virt_move(Centre end)
{	
...
bool Pieces::kill_possible(Centre end)
{
...
bool Pieces::is_valid(Centre end,int n)
{
	
	if(box[end.i][end.j].is_occupied==true)			//Player should move to an Empty Box
		return false;
		
	if((cen.j-end.j)*n<0)							//Player should move only in upward directiion
		return false;
	
	if(end.i<0 or end.j<0 or end.i>7 or end.j>7)	//Player should not move out of the Board
		return false;
		
	if(	abs(cen.i-end.i)==2 and abs(cen.j-end.j)==2 and kill_possible(end))	//Checks validity of a killing move
			return true;
		
	if(abs(cen.i-end.i)!=1 or abs(cen.j-end.j)!=1)		//Player should only move to adjacent black box except for killing move
		return false;
...
{
	for(int n=0;n<12;n++)					//Removes Pieces by changing their radius to 0
	{
...
{					//Checks the possible moves and the outcome of the moves and gives a value based on the analysis.
	k=k+n*20;
...
void Game::start()
{
	for (int i=0;i<8;i++)										//Creating Board
	{
		for(int j=0;j<8;j++)
		{
	...
	for(int i=0;i<8;i++)										//Initializing Boxes of the Board
	{
		for(int j=0;j<8;j++)
		{
			if((i+j)%2==0)
	...
	int k=0;													//Initializing Game Pieces
	for(int j=5;j<8;j++)
	{
		for(int i=0;i<8;i++)
		{
	...
	while(true)													//User's Turn Starts
	{
		...
		check_result(1);		
		for(int z=0;z<12;z++)							//Bringing virtual pieces to same state as the real pieces.
		{
	...
	{													//Computer Turn Starts
		...
		for(int i=0;i<12;i++)							//Choosing the best Killing move.
		{
		...
		if(l==12)											//Choosing the best non-killing move.
		{
			int defeat[24];
			for(int i=0;i<24;i++)
				defeat[i]=0;
		...
		else										//Implementing the best Killing-move
		{
			int max=-1;
			int index=-1;
			for(int i=0;i<12;i++)
...
{
	int user=0;													//No. of unkilled pieces				
	int comp=0;
	int u_king=0;												//No. of pieces in opponents last line
	int c_king=0;
	int u_moves=0;												//No. of possible moves
	int c_moves=0;
	...
	if(u_moves==0 and i==-1)		//User is out of moves.				
		g.result(false);
	
	if(c_moves==0 and i==1)			//Computer is out of moves.
		g.result(true);
}
